...
#accessing list of preprocessing method and their parameters v1 not working
'''def ppm_list():
  df_to_parse= pd.read_json(ppm_filename)
  
  multiDict_to_single=[]
  for i in range(len(df_to_parse)):
    qq = dict(ChainMap(*df_to_parse[i][::-1]))
    multiDict_to_single.append(qq)
    pd.DataFrame(multiDict_to_single).head()
'''

#accessing list of preprocessing method and their parameters v2
import json
with open (ppm_filename) as access_json:
    read_content = json.load(access_json)
    for index in
...
#accessing list of Machine Learning model and their parameters
def mla_list():
  with open (mla_filename) as access_mla_json:
    read_mla_content = json.load(access_mla_json)
